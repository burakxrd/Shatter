...
import pytest
from core.engine import _build_hashcat_cmd


class TestBuildHashcatCmd:
    """Tests for _build_hashcat_cmd()."""
    ...
    def test_basic_wordlist(self):
        cmd = _build_hashcat_cmd("0", self._base_settings())
        assert "-a" in cmd
    ...
    def test_hybrid_mode6(self):
        cmd = _build_hashcat_cmd("1000", self._base_settings(
    ...
    def test_single_rule_backward_compat(self):
        cmd = _build_hashcat_cmd("0", self._base_settings(rule="old.rule"))
        assert "-r" in cmd
        assert "old.rule" in cmd

    def test_optimized_kernel(self):
        cmd = _build_hashcat_cmd("0", self._base_settings(optimized_kernel=True))
        assert "-O" in cmd

    def test_session(self):
        cmd = _build_hashcat_cmd("0", self._base_settings(session_name="test_session"))
        assert "--session" in cmd
        assert "test_session" in cmd

    def test_disable_potfile(self):
        cmd = _build_hashcat_cmd("0", self._base_settings(disable_potfile=True))
        assert "--potfile-disable" in cmd

    def test_status_timer(self):
        cmd = _build_hashcat_cmd("0", self._base_settings())
    ...
    def test_skip_and_limit(self):
        cmd = _build_hashcat_cmd("0", self._base_settings(skip="100", limit="500"))
        assert "-s" in cmd and "100" in cmd
        assert "-l" in cmd and "500" in cmd
