"""ui_potfile.py — Potfile Manager tab (Mixin).
...
class PotfileMixin:
    ...
    def _build_potfile(self, parent) -> None:
        ...
        ctk.CTkButton(
            toolbar, text="🔄 Refresh", font=FONT_BUTTON, fg_color=COLOR_ACCENT,
    ...
    def _potfile_render(self) -> None:
        ...
        # Stats line
        if matched == total:
            stats = f"Showing {end} / {total} entries"
        else:
            stats = f"{end} / {matched} matches ({total} total)"
        stats += f"  •  {_get_potfile_path().name}"
        self._pot_stats_label.configure(text=stats)

        if not page_entries:
            lbl = ctk.CTkLabel(
                self._pot_scroll, text="Potfile is empty or no results.",
        ...
        for i, (hash_val, password) in enumerate(page_entries):
            row_color = "#0F172A" if i % 2 == 0 else "#1E293B"
            ...
            h_display = hash_val if len(hash_val) <= 48 else f"{hash_val[:24]}…{hash_val[-20:]}"
            ...
            ctk.CTkButton(
                rf, text="📋", width=32, height=26, corner_radius=6,
        ...
        if remaining > 0:
            more_text = f"▼ Show {remaining} more entries ({PAGE_SIZE} per page)"
    ...
    def _potfile_clear(self) -> None:
        ...
        if not pf.exists():
            self._term_append("[*] Potfile is already empty.\n")
            return
        confirm = messagebox.askyesno(
            "Clear Potfile",
            f"All cracked password history will be deleted.\n\n{pf}\n\nAre you sure?",
        )
        if confirm:
            try:
                pf.unlink()
                self._potfile_refresh()
                self._term_append("[+] Potfile cleared.\n")
                log.info("Potfile cleared by user.")
            except Exception as e:
                self._term_append(f"[!] Failed to clear potfile: {e}\n")
